...
(function ($) {
  ...
  $.extend(Visualizer.prototype, (function () {
	...
	context.moveTo(12 * this.scale / 40, 0);
    ...
    var CHANGED_COLOR = [0.5, 0, 0];

    var PATH_START_COLOR = [1.0, 0.8, 0.8];

    var PATH_START_RAIDUS = 5;
    ...
    var MIN_PATHES_LINE_WIDTH = 2;
    var MAX_PATHES_LINE_WIDTH = 5;

    var PATHES_INNER_RADIUS = 4;
    var PATHES_OUTER_RADIUS = 7;

    function draw_pathes(x, y) {
      var pathes = this.pathes[y * 100 + x];
	...
	scaled.call(this, function () {
	  ...
	  for (var i = 0, size = pathes.length; i < size; i ++) {
	    var a = pathes[i];

	    var sx = a[0], sy = a[1], path = a[2];

	    var p = size == 1 ? 1 : i / (size - 1);

	    var color = [p * 0.5 + 0.5, 0, 0, 0.667];

            Context.prototype.setRGBColor.apply(context, color);

	    context.setLineWidth(   MAX_PATHES_LINE_WIDTH
				 - (MAX_PATHES_LINE_WIDTH - MIN_PATHES_LINE_WIDTH) * p);

	    context.moveTo(x = sx, y = sy);

	    for (var j = 0, size2 = path.length; j < size2; j ++) {
	      d = path[j] - 0;

	      context.lineTo(x = x + dx[d], y = y + dy[d]);
	    }
	    ...
	    context.matrixexec(this.dm, function () {
	      ...
	      context.arc(p[0], p[1], PATHES_OUTER_RADIUS, 0, 360);
	    ...
	    context.matrixexec(this.dm, function () {
	      ...
	      context.arc(p[0], p[1], PATHES_INNER_RADIUS, 0, 360);
    ...
    var MIN_SEGMENTS_LINE_WIDTH = 2;
    var MAX_SEGMENTS_LINE_WIDTH = 5;

    var SEGMENTS_INNER_RADIUS = 4;
    var SEGMENTS_OUTER_RADIUS = 7;

    function draw_segments(x, y) {
      var segment_indices = this.segment_indices[y * 100 + x];
	...
	scaled.call(this, function () {
	  ...
	  for (var i = 0, size = segment_indices.length; i < size; i ++) {
	    var a = this.segments[segment_indices[i]];

	    var sx = a[0], sy = a[1], path = a[2], ex = a[3], ey = a[4];
	    ...
	    if (this.board[sy][sx] == '*') {
	      color = [0, p * 0.5 + 0.5, 0, 0.667];
	    }
	    else if (this.board[ey][ex] == '*') {
	      color = [0, 0, p * 0.5 + 0.5, 0.667];
	    }
	    else {
	      color = [p * 0.5 + 0.5, 0, 0, 0.667];
	    }

            Context.prototype.setRGBColor.apply(context, color);

	    context.setLineWidth(   MAX_SEGMENTS_LINE_WIDTH
				 - (MAX_SEGMENTS_LINE_WIDTH - MIN_SEGMENTS_LINE_WIDTH) * p);

	    context.moveTo((x = sx) + dx[d] * 0.2, (y = sy) + dy[d] * 0.2);
...
//	      context.newPath();
//	      
//	      context.arc(p[0], p[1], SEGMENTS_OUTER_RADIUS, 0, 360);
//
//	      context.fill();
//	    }.bind(this));
	    ...
	    context.matrixexec(this.dm, function () {
	      ...
	      context.arc(p[0], p[1], SEGMENTS_INNER_RADIUS, 0, 360);
    ...
    var MIN_INCOMMING_LINE_WIDTH = 2;
    var MAX_INCOMMING_LINE_WIDTH = 5;

    var INCOMMING_INNER_RADIUS = 4;
    var INCOMMING_OUTER_RADIUS = 7;

    function draw_incomming(x, y) {
      var incomming = this.incomming[y * 100 + x];
	...
	scaled.call(this, function () {
	  ...
	  for (var i = 0, size = incomming.length; i < size; i ++) {
	    var a = incomming[i];
	    ...
	    var color = [0, p * 0.5 + 0.5, 0];

            Context.prototype.setRGBColor.apply(context, color);

	    context.setLineWidth(   MAX_INCOMMING_LINE_WIDTH
				 - (MAX_INCOMMING_LINE_WIDTH - MIN_INCOMMING_LINE_WIDTH) * p);
	    ...
	    context.matrixexec(this.dm, function () {
	      ...
	      context.arc(p[0], p[1], INCOMMING_OUTER_RADIUS, 0, 360);
	    ...
	    context.matrixexec(this.dm, function () {
	      ...
	      context.arc(p[0], p[1], INCOMMING_INNER_RADIUS, 0, 360);
    ...
    var MIN_OUTGOING_LINE_WIDTH = 2;
    var MAX_OUTGOING_LINE_WIDTH = 5;

    var OUTGOING_INNER_RADIUS = 4;
    var OUTGOING_OUTER_RADIUS = 7;

    function draw_outgoing(x, y) {
      var outgoing = this.outgoing[y * 100 + x];
	...
	scaled.call(this, function () {
	  ...
	  for (var i = 0, size = outgoing.length; i < size; i ++) {
	    var a = outgoing[i];
	    ...
	    var color = [0, 0, p * 0.5 + 0.5];

            Context.prototype.setRGBColor.apply(context, color);

	    context.setLineWidth(   MAX_OUTGOING_LINE_WIDTH
				 - (MAX_OUTGOING_LINE_WIDTH - MIN_OUTGOING_LINE_WIDTH) * p);
	    ...
	    context.matrixexec(this.dm, function () {
	      ...
	      context.arc(p[0], p[1], OUTGOING_OUTER_RADIUS, 0, 360);
	    ...
	    context.matrixexec(this.dm, function () {
	      ...
	      context.arc(p[0], p[1], OUTGOING_INNER_RADIUS, 0, 360);
    ...
    var PATH_COLOR = [1, 0, 1];

    function draw_path(sx, sy, path) {
      scaled.call(this, function () {
	var context = this.context;

	context.translate(0.5, 0.5);

        Context.prototype.setRGBColor.apply(context, PATH_COLOR);
    ...
    function deferred(callback, timeout) {
      var did = ++ DID;
